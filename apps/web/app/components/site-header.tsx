...
export function SiteHeader() {
	return (
		<header className="flex backdrop-blur-sm z-50 shrink-0 fixed w-full lg:relative h-14 items-center gap-2 border-b transition-[width,height] ease-linear group-has-data-[collapsible=icon]/sidebar-wrapper:h-(--header-height)">
		...
		.forEach((match) => {
			...
			currentSegments.forEach((segment) => {
				...
				if (segment.includes(":")) {
					...
					const paramValue = match.params[paramKey];
					label = paramValue || paramKey;
				} else if (
				...
				{breadcrumbs.map((crumb, index) => (
					<BreadcrumbItem key={crumb.path}>
						{index < breadcrumbs.length - 1 ? (
							<>
								<Link prefetch="intent" to={crumb.path} className="text-sm">
									{crumb.label}
								</Link>
								<BreadcrumbSeparator />
							</>
